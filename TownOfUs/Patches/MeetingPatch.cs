...
namespace TownOfUs.Patches
{
    ...
    class MeetingHudPatch
    {
        ...
        static bool IsBlockedBlackmail() => Blackmailer.blackmailed != null && Blackmailer.blackmailed == PlayerControl.LocalPlayer && Blackmailer.blockTargetAbility;
        ...
        class MeetingCalculateVotesPatch
        {
            ...
            {
                if (__instance.playerStates.All((PlayerVoteArea ps) => ps.AmDead || ps.DidVote || Blackmailer.blackmailed != null && Blackmailer.blackmailed.PlayerId == ps.TargetPlayerId && Blackmailer.blockTargetVote))
                {
                    // If skipping is disabled, replace skipps/no-votes with self vote
                    if (target == null && blockSkippingInEmergencyMeetings != BlockOptions.Off && noVoteIsSelfVote)
                    {
                        foreach (PlayerVoteArea playerVoteArea in __instance.playerStates)
                        {
        ...
        class MeetingHudSelectPatch
        {
            ...
            {
                if (Blackmailer.blackmailed != null && Blackmailer.blackmailed == PlayerControl.LocalPlayer && Blackmailer.blockTargetVote) {
                    __result = false;
                    return false;
                }
                return true;
        ...
        class SetHighlightedPatch
        {
            public static bool Prefix(PlayerVoteArea __instance, bool value)
            {
                ...
                __instance.HighlightedFX.enabled = value && __instance.canBeHighlighted() && !(Blackmailer.blackmailed != null && Blackmailer.blackmailed == PlayerControl.LocalPlayer && Blackmailer.blockTargetVote);
                return false;
            }
        }
        ...
        class MeetingHudGetVotesRemainingPatch
        {
            public static bool Prefix(MeetingHud __instance, ref int __result)
            {
                try
                {
                    __result = __instance.playerStates.Count(ps => !ps.AmDead && !ps.DidVote && !(Blackmailer.blackmailed != null && Blackmailer.blackmailed.PlayerId == ps.TargetPlayerId && Blackmailer.blockTargetVote));
                }
                catch
                {
                    __result = 0;
            ...
            private static bool Prefix(MeetingHud __instance, Il2CppStructArray<MeetingHud.VoterState> states)
            {
                ...
                for (int i = 0; i < __instance.playerStates.Length; i++)
                {
                    ...
                    for (int j = 0; j < states.Length; j++)
                    {
                        ...
                        else if (Blackmailer.blackmailed != null && voterState.VoterId == Blackmailer.blackmailed.PlayerId && Blackmailer.blockTargetVote)
                            continue;
        ...
        public static void swapperCheckAndReturnSwap(MeetingHud __instance, byte dyingPlayerId)
        {
            ...
            Swapper.charges++;  // Swapper had previously swapped, refund one charge!
            meetingExtraButtonText.text = $"Swaps: {Swapper.charges}";
            meetingExtraButtonLabel.text = Helpers.cs(Color.red, "Confirm Swap");
        }
        ...
        static void amnesiacRemember(int i, MeetingHud __instance)
        {
            ...
            AmongUsClient.Instance.FinishRpcImmediately(writer);
            RPCProcedure.amnesiacTakeRole(__instance.playerStates[i].TargetPlayerId, PlayerControl.LocalPlayer.PlayerId);
        ...
        static void guesserOnClick(int buttonTarget, MeetingHud __instance)
        {
            ...
            foreach (RoleInfo roleInfo in RoleInfo.allRoleInfos)
            {
                ...
                if (!PlayerControl.LocalPlayer.canGuess(roleInfo)) continue;

                if (Snitch.snitch != null && PlayerControl.LocalPlayer.cantGuessSnitch())
                {
                    if (TasksHandler.taskInfo(Snitch.snitch.Data).Item2 - TasksHandler.taskInfo(Snitch.snitch.Data).Item1 <= 0 && roleInfo.roleId == RoleId.Snitch) continue;
                }
            ...
            void CreateRole(RoleInfo roleInfo)
            {
                ...
                if (!PlayerControl.LocalPlayer.Data.IsDead && Helpers.playerById(__instance.playerStates[buttonTarget].TargetPlayerId) != null && !Helpers.playerById(__instance.playerStates[buttonTarget].TargetPlayerId).Data.IsDead) button.GetComponent<PassiveButton>().OnClick.AddListener((System.Action)(() =>
                {
                    ...
                    else
                    {
                        ...
                        if (!PlayerControl.LocalPlayer.killsThroughShield() && Medic.shielded == focusedTarget)
                        {
                        ...
                        if (Indomitable.indomitable != null && Indomitable.indomitable == focusedTarget)
                        {
                            __instance.playerStates.ToList().ForEach(x => x.gameObject.SetActive(true));
                            UnityEngine.Object.Destroy(container.gameObject);
                        ...
                        if (Doomsayer.doomsayer != null && Doomsayer.doomsayer == PlayerControl.LocalPlayer && dyingTarget == PlayerControl.LocalPlayer)
                        {
                            ...
                            Helpers.showFlash(Doomsayer.color, 1f, "Wrong guess");
                            return;
                        }

                        if (DoubleShot.doubleShot != null && DoubleShot.doubleShot == PlayerControl.LocalPlayer && dyingTarget == PlayerControl.LocalPlayer && !DoubleShot.isUsedSecondLife)
                        {
                            dyingTarget = null;
                            __instance.playerStates.ToList().ForEach(x => x.gameObject.SetActive(true));
                            UnityEngine.Object.Destroy(container.gameObject);
                            __instance.playerStates.ToList().ForEach(x => { if (x.TargetPlayerId == focusedTarget.PlayerId && x.transform.FindChild("ShootButton") != null) UnityEngine.Object.Destroy(x.transform.FindChild("ShootButton").gameObject); });
                            DoubleShot.isUsedSecondLife = true;
                            Helpers.showFlash(DoubleShot.color, 1f, "Extra life used");
                            return;
                        }
                        ...
                        __instance.playerStates.ToList().ForEach(x => x.gameObject.SetActive(true));
                        UnityEngine.Object.Destroy(container.gameObject);
                        if (PlayerControl.LocalPlayer.hasMultipleShotsPerMeeting() && HandleGuesser.remainingShots(PlayerControl.LocalPlayer.PlayerId) > 1 && dyingTarget != PlayerControl.LocalPlayer)
        ...
        {
            // Add Swapper Buttons
            bool addSwapperButtons = Swapper.swapper != null && PlayerControl.LocalPlayer == Swapper.swapper && !Swapper.swapper.Data.IsDead && !IsBlockedBlackmail();
            ...
            if (PlayerControl.LocalPlayer.IsAmnesiac(out _) && !PlayerControl.LocalPlayer.Data.IsDead && Amnesiac.rememberMeeting && HandleGuesser.isGuesser(PlayerControl.LocalPlayer.PlayerId) && !IsBlockedBlackmail())
            {
                for (int i = 0; i < __instance.playerStates.Length; i++)
                {
                    PlayerVoteArea playerVoteArea = __instance.playerStates[i];
        ...
        class StartMeetingPatch
        {
            public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo meetingTarget)
            {
                ...
                {
                    TownOfUsPlugin.Logger.LogWarning("sharing room on meeting start");
                ...
                // Save Immovable position, if the player is able to move (i.e. not on a ladder or a gap thingy)
                if (PlayerControl.LocalPlayer.MyPhysics.enabled && (PlayerControl.LocalPlayer.moveable || PlayerControl.LocalPlayer.inVent || spyVitalsButton != null && spyVitalsButton.isEffectActive || spyAdminTableButton != null && spyAdminTableButton.isEffectActive))
                {
                ...
                // Lookout Reset Zoom
                if (Lookout.lookout != null && Lookout.zoomOutStatus)
                {
                ...
                // Add Snitch info
                string output = "";
                if (Snitch.snitch != null && Snitch.mode != Snitch.Mode.Map && Snitch.mode != Snitch.Mode.Arrows && (PlayerControl.LocalPlayer == Snitch.snitch || Helpers.shouldShowGhostInfo()) && !Snitch.snitch.Data.IsDead)
                {
                    ...
                    if (numberOfTasks == 0)
                    {
                        ...
                        FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(0.4f, new Action<float>((x) =>
                        {
                            if (x == 1f)
                            {
                                foreach (PlayerControl p in PlayerControl.AllPlayerControls.ToArray().OrderBy(x => Guid.NewGuid()))
                                {
                ...
                if (Trapper.trapper != null && PlayerControl.LocalPlayer == Trapper.trapper && !PlayerControl.LocalPlayer.Data.IsDead)
                {
                    if (Trapper.trappedPlayers.Count == 0)
                    {
                        FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "No players entered any of your traps");
                        return;
                    }

                    if (Trapper.trappedPlayers.Count < Trapper.playersInTrap)
                    {
                        FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "Not enough players triggered your traps");
                        return;
                    }

                    string message = "Roles caught in your trap:\n";
                    foreach (RoleInfo roleInfo in Trapper.trappedPlayers.OrderBy(x => Guid.NewGuid()))
                    {
                        message += $" {roleInfo.name},";
                    }
                ...
                // Add Oracle Info
                if (Oracle.oracle != null && Oracle.confessor != null && PlayerControl.LocalPlayer == Oracle.oracle && !Oracle.oracle.Data.IsDead)
                {
        ...
        class MeetingHudUpdatePatch
        {
            ...
            static void Postfix(MeetingHud __instance)
            {
                ...
                if (target == null && blockSkippingInEmergencyMeetings == BlockOptions.Emergency || blockSkippingInEmergencyMeetings == BlockOptions.Always)
                ...
                if (Blackmailer.blackmailed != null)
                {
                    ...
                    var playerState = __instance.playerStates.FirstOrDefault(x => Blackmailer.blackmailed.PlayerId == x.TargetPlayerId);
        ...
        public class BlockQuickChatAbility
        {
            ...
            {
                if (Blackmailer.blackmailed != null && PlayerControl.LocalPlayer == Blackmailer.blackmailed) {
                    return false;
                }
                return true;
            }
        ...
        {
            static private Sprite LightColorSprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.ColorLight.png", 100f);
            static private Sprite DarkColorSprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.ColorDark.png", 100f);
            static void Postfix(MeetingHud __instance)
            {
            ...
            public static void GenButton(PlayerVoteArea voteArea)
            {
                ...
                var newButton = UnityEngine.Object.Instantiate(colorButton, voteArea.transform);
                ...
                bool isLighter = Helpers.isLighterColor(playerControl);

                if (isLighter) renderer.sprite = LightColorSprite;
                else renderer.sprite = DarkColorSprite;
