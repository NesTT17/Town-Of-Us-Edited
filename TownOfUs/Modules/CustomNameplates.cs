...
namespace TownOfUs.Modules
{
    ...
    public class CustomNamePlates
    {
        ...
        {
            string filePath = Path.GetDirectoryName(Application.dataPath) + @"\TownOfPlates\";
    ...
    public class CustomNameplateLoader
    {
        ...
        private const string REPO_SV = "https://raw.githubusercontent.com/NesTT17/TheOtherHats/master";
        ...
        private static async Task LaunchNameplateFetcherAsync()
        {
            try
            {
                ...
                if (status != HttpStatusCode.OK)
                    TownOfUsPlugin.Logger.LogWarning("Custom Nameplates could not be loaded");
            }
            catch (Exception e)
            {
                TownOfUsPlugin.Logger.LogError("Unable to fetch Nameplates: " + e.StackTrace);
            }
            IsRunning = false;
        }
        ...
        public static async Task<HttpStatusCode> FetchNameplates_SN()
        {
            ...
            try
            {
                ...
                {
                    TownOfUsPlugin.Logger.LogWarning("Server returned no data (fetching Nameplates): " + response.StatusCode.ToString());
                ...
                else
                {
                    for (int i = 0; i < markedForDownload.Count; i++)
                    {
                        ...
                        TownOfUsPlugin.Logger.LogMessage(file + " Downloaded");
            ...
            {
                TownOfUsPlugin.Logger.LogError("Error fetching Nameplate metadata: " + ex.ToString());
            }
