...
using static TownOfUs.CustomOption;
...
namespace TownOfUs {
    public class CustomOption {
        ...
        public CustomOption(int id, CustomOptionType type, string name,  System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, Action onChange = null, string heading = "") {
            ...
            selection = 0;
            if (id != 0) {
                entry = TownOfUsPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
        ...
        public static void switchPreset(int newPreset) {
            ...
            vanillaSettings = TownOfUsPlugin.Instance.Config.Bind($"Preset{preset}", "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options) {
                if (option.id == 0) continue;

                option.entry = TownOfUsPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
        ...
        public static int deserializeOptions(byte[] inputValues) {
            ...
            while (reader.BaseStream.Position < inputValues.Length) {
                try {
                    ...
                    TownOfUsPlugin.Logger.LogWarning($"id:{lastId}:{e}: while deserializing - tried to paste invalid settings!");
                    errors++;
                }
            }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                ShareOptionSelections();
                if (TownOfUsPlugin.Version > versionInfo && versionInfo < Version.Parse("4.6.0")) {
    ...
    class LobbyViewSettingsPatch{
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) {
            ...
            var relevantOptions = options.Where(x => x.type == optionType || x.type == CustomOption.CustomOptionType.NeutralKiller && optionType == CustomOptionType.Neutral).ToList();
            ...
            if (TOUMapOptions.gameMode == CustomGamemodes.Guesser)
                relevantOptions = relevantOptions.Where(x => !(new List<int> { 310, 311, 312, 313, 314, 315, 316 }).Contains(x.id)).ToList();
                
            if (TOUMapOptions.gameMode == CustomGamemodes.Classic)
                relevantOptions = relevantOptions.Where(x => !(new List<int> { 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008 }).Contains(x.id)).ToList();
            ...
            foreach (var option in relevantOptions) {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type) {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOptionType, string>() { { CustomOptionType.Crewmate, "Crewmate Roles" }, { CustomOptionType.Neutral, "Neutral Non-Killing Roles" }, { CustomOptionType.NeutralKiller, "Neutral Killing Roles" },
                            { CustomOptionType.Impostor, "Impostor Roles" },  { CustomOptionType.Modifier, "Modifiers" } }[curType];
        ...
        private static Tuple<string, string> handleSpecialOptionsView(CustomOption option, string defaultString, string defaultVal) {
            ...
            if (option == CustomOptionHolder.killingNeutralRolesCountMin) {
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance) {
            ...
            int next = 3;
            if (TOUMapOptions.gameMode == CustomGamemodes.Classic || TOUMapOptions.gameMode == CustomGamemodes.Guesser) {
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "TOU Settings", CustomOptionType.General);
                // create TOR settings
    ...
    class GameOptionsMenuStartPatch {
        ...
        public static void Postfix(GameSettingMenu __instance) {
            ...
            copyButtonRenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.PasteActive.png", 100f);
        ...
        private static void createSettingTabs(GameSettingMenu __instance) {
            ...
            if (TOUMapOptions.gameMode == CustomGamemodes.Classic || TOUMapOptions.gameMode == CustomGamemodes.Guesser) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "TOU Settings");
                ...
                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "Neutral NK Roles");
                ...
                createGameOptionsMenu(__instance, CustomOptionType.NeutralKiller, "NeutralKSettings");
                // IMp
    ...
    class GameOptionsDataPatch
    {
        private static string buildRoleOptions() {
            ...
            var kneutralRoles = buildOptionsOfType(CustomOption.CustomOptionType.NeutralKiller, true) + "\n";
            ...
            return crewRoles + neutralRoles + kneutralRoles + impRoles + modifiers;
        }
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            if (TOUMapOptions.gameMode == CustomGamemodes.Guesser) {
                List<int> remove = new List<int>{ 310, 311, 312, 313, 314, 315, 316 };
                options = options.Where(x => !remove.Contains(x.id));
            } else if (TOUMapOptions.gameMode == CustomGamemodes.Classic) {
                List<int> remove = new List<int>{ 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008 };
                options = options.Where(x => !remove.Contains(x.id));
            }
            ...
            foreach (CustomOption option in options) {
                if (option.parent != null) {
                    ...
                    } else if (option == CustomOptionHolder.killingNeutralRolesCountMin) {
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            switch (counter) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    hudString += "Page 5: Neutral Non-Killing Role Settings \n" + buildOptionsOfType(CustomOption.CustomOptionType.Neutral, false);
                    break;
                case 5:
                    hudString += "Page 6: Neutral Killing Role Settings \n" + buildOptionsOfType(CustomOption.CustomOptionType.NeutralKiller, false);
                    break;
                case 6:
    ...
    public class HudManagerUpdate {
        ...
        public static void Prefix3(HudManager __instance) {
            ...
            var blocks = summaryString.Split("\n\n", StringSplitOptions.RemoveEmptyEntries); ;
            string curString = "";
            string curBlock;
            int j = 0;
            for (int i = 0; i < blocks.Length; i++) {
            ...
            for (int i = 0; i < blockCount; i++) {
                summaryTMPs[i].transform.localPosition = new Vector3(- blockCount * 1.2f + 2.7f * i, 2.2f, -500f);
            }
        }
        ...
        public static void Postfix(HudManager __instance) {
            ...
            if (!toggleSettingsButton || !toggleSettingsButtonObject) {
                ...
                renderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Settings_ButtonActive.png", 100);
            ...
            if (!toggleZoomButton || !toggleZoomButtonObject) {
                ...
                tZrenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Helpers.loadSpriteFromResources("TownOfUs.Resources.Minus_ButtonActive.png", 100);
            ...
            toggleZoomButtonObject.SetActive(PlayerControl.LocalPlayer != null && PlayerControl.LocalPlayer.Data.IsDead && !LobbyBehaviour.Instance && !ExileController.Instance && !MeetingHud.Instance);
            toggleZoomButtonObject.transform.localPosition = __instance.MapButton.transform.localPosition + new Vector3(0, -2.4f, -500f);
        }
    }
}
